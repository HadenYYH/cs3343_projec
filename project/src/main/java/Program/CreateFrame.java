 ...
 // method actionPerformed()
    ...
    public void actionPerformed(ActionEvent e)
    {
        if (e.getSource() == sub) {
        	...
        	String message = Topics.getInstance().createTopic(user, nameText, descriptionText);
        	if(message(message).startsWith("Topic created")) {
        		String id = message.substring(message.length() - 8);
        		new TopicFrame(user, Topics.getInstance().getTopic(id));
                this.dispose();
        	}
        }
